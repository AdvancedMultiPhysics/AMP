...
std::shared_ptr<AMP::Operator::OperatorParameters> BDFRadDifOp::getJacobianParameters( AMP::LinearAlgebra::Vector::const_shared_ptr u_in ) {
 ...
 *  ----------------------------------------------------------------------------- */
BDFRadDifOpPJac::BDFRadDifOpPJac( std::shared_ptr<AMP::Operator::OperatorParameters> params ) : AMP::Operator::LinearOperator( params ) {
                ...
                AMP::LinearAlgebra::Vector::shared_ptr r )
{
    ...
    // We have modified the Jacobian data of our RadDifOpPJac operator in such a way that its apply will in fact be an apply of a BDFRadDifOpPJac, but we need to explicitly let it know that we really do intend to do an apply with the current state of its data
    d_RadDifOpPJac->applyWithOverwrittenDataIsValid();
    d_RadDifOpPJac->apply( u_in, r );
}


std::shared_ptr<AMP::LinearAlgebra::Vector> BDFRadDifOpPJac::createInputVector() const { return d_RadDifOpPJac->createInputVector( ); };
...
void BDFRadDifOpPJac::setData() {
    d_data = std::make_shared<AMP::Operator::BDFRadDifOpPJacData>( d_RadDifOpPJac->d_data, d_gamma );
}
