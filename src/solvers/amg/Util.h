...
struct par_csr
{
	...
	par_csr() :
		d_diag{std::make_shared<seq_type>()},
		d_offd{std::make_shared<seq_type>()} {}
...
struct coarse_operator : AMP::Operator::LinearOperator {
	...
	explicit coarse_operator( std::shared_ptr<AMP::Operator::OperatorParameters> params ) :
        ...
        setMatrix( create_matrix( cop->matrix.comm, cop->matrix.diag_extents,
                                  cop->matrix.left_var, cop->matrix.right_var ) );
	...
	create_matrix( const AMP_MPI & comm,
	               ...
	               std::shared_ptr<LinearAlgebra::Variable> left_var, std::shared_ptr<LinearAlgebra::Variable> right_var ) {
		...
		auto params = std::make_shared<LinearAlgebra::RawCSRMatrixParameters<Config>>(
			diag_extents[0], diag_extents[1],
			diag_extents[0], diag_extents[1],
			diag_params, offd_params, comm, left_var, right_var);
...
struct csr_view<LinearAlgebra::CSRMatrix<Config>>
{
	...
	[[nodiscard]] size_t numLocalRows() const {
...
protected:
	...
	auto csr_ptrs(const T & local_data) const {
		...
		return std::make_tuple(
			span<const lidx_t>{rowptr, static_cast<size_t>(nrows + 1)},
...
private:
	auto csr_ptrs(const typename value_type::seq_type & v) const {
		return std::make_tuple(
			span<const lidx_t>(v.rowptr.data(), v.rowptr.size()),
