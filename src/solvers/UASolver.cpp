...
UASolver::UASolver( std::shared_ptr<SolverStrategyParameters> params ) :
...
void UASolver::getFromInput( std::shared_ptr<AMP::Database> db )
{
	...
	d_coarsen_settings.redist_coarsen_factor = db->getWithDefault<size_t>( "redist_coarsen_factor", 2 );
...
void UASolver::setup()
{
	...
	for (size_t i = 0; i < d_max_levels; ++i) {
		...
		if ( num_rows(Ac) <= d_coarsen_settings.min_coarse ) break;
	}
...
void UASolver::apply( std::shared_ptr<const LinearAlgebra::Vector> b,
	...
	for (int i = 0; i < d_iMaxIterations; ++i) {
		...
		A.residual( b, x, r );
		auto rnorm = r->L2Norm();
		if (rnorm <= d_dRelativeTolerance) break;
	}
}
