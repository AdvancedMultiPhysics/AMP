...
{
    if ( memory_location <= MemoryType::host ) {
#ifdef AMP_USE_OPENMP
        return Backend::OpenMP;
#else
        return Backend::Serial;
#endif
    } else if ( memory_location >= MemoryType::managed ) {
...
{
    if ( backend == Backend::Serial ) {
        return memory_location <= MemoryType::host;
    }
#ifdef AMP_USE_DEVICE
    if ( backend == Backend::Hip_Cuda ) {
        return memory_location >= MemoryType::managed;
    }
#endif
#ifdef AMP_USE_KOKKOS
    if ( backend == Backend::Kokkos ) {
        // compatible with all spaces
        return true;
    }
#endif
#ifdef AMP_USE_KOKKOS
